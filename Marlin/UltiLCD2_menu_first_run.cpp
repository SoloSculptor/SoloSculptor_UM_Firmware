...
#define BED_CENTER_ADJUST_Y ((int)Y_MAX_LENGTH - 10) //linanw to-do: test first run, if plate level center position is right. 
...
static void homeAndParkHeadForCenterAdjustment2()
{
    ...
    char buffer[32];
    sprintf_P(buffer, PSTR("G1 F%i Z%i X%i Y%i"), int(homing_feedrate[0]), 35, X_MAX_LENGTH/2, BED_CENTER_ADJUST_Y); //linanw to-do: test if plate leveling center position is right.
    enquecommand(buffer);
}
...
static void parkHeadForCenterAdjustment()
{
    ...
    sprintf_P(buffer, PSTR("G1 F%i Z5"), int(homing_feedrate[Z_AXIS]));
    enquecommand(buffer);
    sprintf_P(buffer, PSTR("G1 F%i X%i Y%i"), int(homing_feedrate[X_AXIS]), X_MAX_LENGTH / 2, BED_CENTER_ADJUST_Y); //linanw to-do: check if plate leveling paper round toolhead center position is right. 
    enquecommand(buffer);
    sprintf_P(buffer, PSTR("G1 F%i Z0"), int(homing_feedrate[Z_AXIS]));
    enquecommand(buffer);
}
...
static void lcd_menu_first_run_bed_level_paper_right()
{
    ...
    if (IS_FIRST_RUN_DONE())
        ...
        lcd_info_screen(lcd_menu_first_run_material_load, homeBed, PSTR("CONTINUE")); //linan to-do: need to test if XY is home after plate leveling.
